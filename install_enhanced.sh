...
echo -e "${GREEN}Starting VPS VPN Auto-Installer V2.1 Enhanced (Traffic & Speedtest)...${PLAIN}"

# 1. System Update & Dependencies
echo -e "${YELLOW}[1/6] Updating System & Installing Dependencies...${PLAIN}"
apt update && apt install -y curl wget git nginx certbot python3-certbot-nginx unzip uuid-runtime apache2-utils qrencode php-fpm php-cli php-xml sudo cron jq vnstat speedtest-cli
...
# If DOMAIN is already set (e.g. passed as env var or arg), use it.
if [ -z "$DOMAIN" ]; then
...
echo "www-data ALL=(root) NOPASSWD: /usr/bin/speedtest-cli" >> /etc/sudoers.d/vpanel
...
if(isset($_POST['action'])) {
    ...
    elseif($_POST['action'] === 'speedtest') {
        $speedtest_result = shell_exec('sudo speedtest-cli --simple 2>&1');
    }
...
// Try to get JSON output, fallback to text
$vnstat_json = shell_exec("vnstat --json 2>/dev/null");
$traffic_data = null;
if($vnstat_json) {
    $vj = json_decode($vnstat_json, true);
    // Find the interface with traffic
    if (isset($vj['interfaces'][0])) {
        $traffic_data = $vj['interfaces'][0]['traffic']['total'];
    }
}
...
$rx_total = isset($traffic_data['rx']) ? formatBytes($traffic_data['rx']) : '0 B';
$tx_total = isset($traffic_data['tx']) ? formatBytes($traffic_data['tx']) : '0 B';
    ...
    <div class="container">
        ...
        <div class="card">
            ...
            <div class="stats-grid">
                ...
                <!-- Traffic Stats -->
                <div class="stat-item">
                    <span class="stat-label">Total Traffic RX</span>
                    <span class="stat-val" style="color: #0f0;"><?php echo $rx_total; ?></span>
                </div>
                <div class="stat-item">
                    <span class="stat-label">Total Traffic TX</span>
                    <span class="stat-val" style="color: #ffd700;"><?php echo $tx_total; ?></span>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            ...
            <p style="color:#aaa; font-size:0.9rem;">Run a network speed test on the VPS.</p>
            <form method="POST">
                <input type="hidden" name="action" value="speedtest">
                <button type="submit" class="btn-blue" style="width: 200px;">RUN SPEEDTEST</button>
            </form>
            <?php if($speedtest_result): ?>
                <div style="margin-top: 15px;">
                    <strong>Result:</strong>
                    <pre><?php echo htmlspecialchars($speedtest_result); ?></pre>
                </div>
            <?php endif; ?>
        </div>
